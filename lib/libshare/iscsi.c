		...
		int check_len)
{
	...
	if ((dir = opendir(path))) {
		while ((directory = readdir(dir))) {
			...
			entries = (iscsi_dirs_t *)
				malloc(sizeof (iscsi_dirs_t));
			if (entries == NULL)
				goto look_out;

			if (path3)
				strncpy(entries->path, path3,
					sizeof (entries->path));
			strncpy(entries->entry, directory->d_name,
				sizeof (entries->entry));
			entries->stats = eStat;
...
iscsi_write_sysfs_value(char *path, char *value)
{
	...
	fprintf(stderr, "iscsi_write_sysfs_value: %s\n"
		"                         => %s\n", full_path, value);
	rc = SA_OK; /* We're debugging, so exit here and fake success */
#endif
...
/*
 * Reads the file and register if a tid have a sid. Save the value in
 * iscsi_targets->state
 */
...
iscsi_retrieve_sessions_scst(void)
{
	...
	while (entries1 != NULL) {
		...
		while (entries2 != NULL) {
			...
			entries3 = iscsi_look_for_stuff(entries2->path, NULL,
							B_TRUE, 4);
			if (entries3 == NULL)
				goto iscsi_retrieve_sessions_scst_error;
			_entries3 = entries3;
			while (entries3 != NULL) {
				...
				session = (iscsi_session_t *)
					malloc(sizeof (iscsi_session_t));
				if (session == NULL)
					exit(SA_NO_MEMORY);
...
iscsi_retrieve_targets_scst(void)
{
	...
	_entries1 = entries1;
	while (entries1 != NULL) {
		entries2 = iscsi_look_for_stuff(entries1->path, "iqn.",
						B_TRUE, 4);
		if (entries2 == NULL)
...
#ifdef DEBUG
	fprintf(stderr, "iscsi_enable_share_one: name=%s, tid=%d, "
...
#ifdef DEBUG
	fprintf(stderr, "iscsi_enable_share_one: name=%s, iomode=%s, type=%s, "
	...
	while (iscsi_targets != NULL) {
		if (iscsi_targets->tid == tid) {
			...
			fprintf(stderr, "iscsi_disable_share_one_scst: "
				...
				iscsi_targets->name, iscsi_targets->tid,
				iscsi_targets->path, iscsi_targets->iotype);
#endif
...
iscsi_disable_share(sa_share_impl_t impl_share)
{
	...
	while (iscsi_targets != NULL) {
		if (strcmp(impl_share->sharepath, iscsi_targets->path) == 0) {
			...
			if (iscsi_targets->session &&
			    iscsi_targets->session->state) {
				/*
				 ...
				 */
				fprintf(stderr, "Can't unshare - already "
					"active with shares\n");
				return (SA_OK);
			}
...
iscsi_disable_share_all(void)
{
	...
	while (iscsi_targets != NULL) {
#ifdef DEBUG
		fprintf(stderr, "iscsi_disable_share_all: target=%s, "
			"tid=%d, path=%s\n", iscsi_targets->name,
			iscsi_targets->tid, iscsi_targets->path);
#endif
    ...
    const char *shareopts)
{
	...
	if (strcmp(shareopts, "on") == 0 || strncmp(shareopts, "name=", 5)
	    != 0) {
		/*
