...
static smb_shares_list_t *
smb_share_list_alloc(void)
{
	smb_shares_list_t *share;

	share = (smb_shares_list_t *) malloc(sizeof (smb_shares_list_t));
	if (share == NULL)
		return (NULL);

	list_link_init(&share->next);
...
//	}
	...
	list_create(&shares_list, sizeof (smb_shares_list_t),
		    offsetof(smb_shares_list_t, next));
	...
	while (fgets(buffer, sizeof (buffer), sharesmb_temp_fp) != 0) {
		...
		share = smb_share_list_alloc();
		if (share == NULL) {
			rc = SA_NO_MEMORY;
			goto smb_retrieve_shares_out;
		}

		strncpy(share->name, dup_value, sizeof (share->name));

		list_insert_tail(&shares_list, share);
	}
	...
	for (share = list_head(&shares_list); share != NULL;
	     share = list_next(&shares_list, share))
	{
		share_info = smb_retrieve_share_info(share->name);
