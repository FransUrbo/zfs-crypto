...
_zed_exec_fork_child(uint64_t eid, const char *dir, const char *prog,
    char *env[], const char *class, int zfd)
{
	char path[PATH_MAX];
	int n;
	pid_t pid;
	...
	if (pid < 0) {
		...
		zed_log_msg(LOG_INFO, "Invoking \"%s\" eid=%llu pid=%d class=%s",
		    prog, eid, pid, class);
...
restart:
		...
		if (wpid == (pid_t) -1) {
			...
			zed_log_msg(LOG_INFO,
			    "Finished \"%s\" eid=%llu pid=%d class=%s exit=%d",
			    prog, eid, pid, class, WEXITSTATUS(status));
		} else if (WIFSIGNALED(status)) {
			zed_log_msg(LOG_INFO,
			    "Finished \"%s\" eid=%llu pid=%d class=%s sig=%d/%s",
			    prog, eid, pid, class, WTERMSIG(status),
			    strsignal(WTERMSIG(status)));
		} else {
			zed_log_msg(LOG_INFO,
			    "Finished \"%s\" eid=%llu pid=%d class=%s status=0x%X",
			    prog, eid, class, (unsigned int) status);
		}
	}
}
	...
	for (s = zed_strings_first(scripts); s; s = zed_strings_next(scripts)) {
		for (csp = class_strings; *csp; csp++) {
			...
			if ((strncmp(s, *csp, n) == 0) && !isalpha(s[n]))
				_zed_exec_fork_child(eid, dir, s, e, *csp, zfd);
		}
	}
	free(e);
	return (0);
